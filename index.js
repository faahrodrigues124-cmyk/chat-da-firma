...
export class ChatRoom {
  constructor(state, env) {
    ...
    this.mensagens = [];
    this.fila = [];
    this.tocandoAgora = null; // { id, title, channel, thumbnail, startedAt }
  ...
  iniciarSessao(socket) {
    const sessao = { nome: "Visitante" };
    this.sessoes.set(socket, sessao);

    // Envia o estado atual da sala assim que o usuário conecta.
  ...
  transmitirPresenca() {
    const usuarios = Array.from(this.sessoes.values()).map((s) => s.nome);
  ...
  async tratarMensagem(socket, dadoBruto) {
    ...
    switch (dados.type) {
      case "entrar": {
        const nome = String(dados.nome || "").trim().slice(0, MAX_NOME);
        sessao.nome = nome || "Visitante";
        this.transmitirPresenca();
        break;
      }
      ...
      case "fila_adicionar": {
        ...
        if (!this.tocandoAgora) {
          this.tocandoAgora = { ...musica, startedAt: Date.now() };
      ...
      case "proxima_musica": {
        ...
        this.tocandoAgora = this.fila.length > 0
          ? { ...this.fila.shift(), startedAt: Date.now() }
          : null;
