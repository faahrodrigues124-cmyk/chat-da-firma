...
async function boot() {
  ...
  updateRadioButton(); bindMediaInputs(); imageButton.innerHTML = icon("image", 17); $("open-recap").innerHTML = `${icon("chart", 12)}<span>Recap</span>`;
...
async function searchMusic() {
  ...
  try { const { r, data } = await fetchJsonSafe(`/api/music?q=${encodeURIComponent(q)}`); if (!r.ok || !data.ok) { musicStatus.textContent = data.error || "Não foi possível buscar."; return; } musicStatus.textContent = data.results.length ? `${data.results.length} resultados` : "Nenhum resultado."; data.results.forEach(addSearchResult); }
  catch { musicStatus.textContent = "Falha de rede."; }
}
async function searchAndQueue(q) {
  if (!q) return false;
  try { const { r, data } = await fetchJsonSafe(`/api/music?q=${encodeURIComponent(q)}`); if (!r.ok || !data.ok || !data.results?.length) { toast(data.error || "Não encontrei essa música.", true); return false; } return queueTrack(data.results[0]); }
