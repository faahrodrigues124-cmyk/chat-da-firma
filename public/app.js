...
const playlistLibraryList = $("playlist-library-list");
const playlistLibraryStatus = $("playlist-library-status");
const playlistHistoryList = $("playlist-history-list");
const playlistPicker = $("playlist-picker");
const playlistRefreshNow = $("playlist-refresh-now");
...
async function boot() {
  ...
  $("open-playlists").onclick = openPlaylists;
  ...
  $("playlist-close").onclick = () => playlistModal.classList.add("hidden");
  playlistRefreshNow.onclick = () => { if (activePlaylistId) sendWS({ type: "playlist_refresh_now", playlistId: activePlaylistId }); };
...
function connectRoom(name) {
  ...
  authenticated = false; messages.innerHTML = ""; updateChatScrollButton(); currentTrack = null; queue = []; loadedVideoId = null; playlistLibrary = {}; playlistHistory = {}; activePlaylistId = null; updateRadio();
...
function handleServer(d) {
  if (d.type === "autenticacao_necessaria") {
    if (d.protegida && !d.autorizado) { passwordError.textContent = ""; roomEnterPass.value = ""; passwordModal.classList.remove("hidden"); roomEnterPass.focus(); }
  ...
  if (d.type === "estado_inicial") {
    ...
    queue = d.fila || []; playlistLibrary = d.playlistLibrary || {}; playlistHistory = d.playlistHistory || {}; activePlaylistId = Object.keys(playlistLibrary)[0] || null; applyRadioState(d.tocandoAgora, d.fila, true); scrollBottom(true);
  ...
  if (d.type === "playlist_atualizada") { if (d.playlistLibrary) playlistLibrary = d.playlistLibrary; if (d.playlistHistory) playlistHistory = d.playlistHistory; if (!activePlaylistId || !playlistLibrary[activePlaylistId]) activePlaylistId = Object.keys(playlistLibrary)[0] || null; queue = Array.isArray(d.fila) ? d.fila : queue; updateMiniQueue(); if (playlistModal && !playlistModal.classList.contains("hidden")) { renderPlaylistPicker(); if (activePlaylistId) renderPlaylistLibrary(activePlaylistId); } return; }
  if (d.type === "playlist_biblioteca") { if (d.playlist?.playlistId) { playlistLibrary[d.playlist.playlistId] = d.playlist; activePlaylistId = d.playlist.playlistId; playlistHistory[d.playlist.playlistId] = d.history || []; renderPlaylistLibrary(activePlaylistId); } return; }
...
function renderPlaylistPicker() {
  ...
  const ids = Object.keys(playlistLibrary || {});
  playlistPicker.innerHTML = "";
  if (!ids.length) {
    playlistPicker.classList.remove("hidden");
    playlistPicker.innerHTML = '<div class="playlist-empty"><strong>Nenhuma playlist adicionada nesta sala.</strong><span>Pesquise ou cole o link de uma playlist em “Adicionar música”.</span></div>';
    return;
  }
  playlistPicker.classList.remove("hidden");
  const title = document.createElement("div"); title.className = "playlist-picker-title"; title.textContent = "PLAYLISTS DA SALA"; playlistPicker.appendChild(title);
  ids.forEach(pid => {
    const lib = playlistLibrary[pid];
    const row = document.createElement("button"); row.type = "button"; row.className = `playlist-picker-row${pid === activePlaylistId ? " active" : ""}`;
    const img = document.createElement("img"); img.src = lib.tracks?.[0]?.thumbnail || ""; img.alt = "";
    const info = document.createElement("span"); info.className = "playlist-picker-info";
    const strong = document.createElement("strong"); strong.textContent = lib.playlistTitle || "Playlist";
    const small = document.createElement("small"); small.textContent = `${lib.tracks?.length || 0} músicas · ${(playlistHistory[pid] || []).length} tocadas`;
    info.append(strong, small); row.append(img, info);
    row.onclick = () => { activePlaylistId = pid; renderPlaylistPicker(); renderPlaylistLibrary(pid); };
...
function openPlaylists() {
  ...
  const ids = Object.keys(playlistLibrary || {});
  if (!activePlaylistId || !playlistLibrary[activePlaylistId]) activePlaylistId = ids[0] || null;
  renderPlaylistPicker();
  if (activePlaylistId) renderPlaylistLibrary(activePlaylistId);
  else { playlistModalTitle.textContent = "Biblioteca de playlists"; playlistLibraryStatus.textContent = "Adicione uma playlist para começar."; playlistLibraryList.innerHTML = ""; playlistHistoryList.innerHTML = ""; }
}
function openMusic() {
  ensureAudio();
  enableMusicAudio();
...
function addPlaylistResult(item) {
  ...
  const img = document.createElement("img"); img.src = item.thumbnail || ""; img.alt = "";
  const info = document.createElement("div"); const b = document.createElement("strong"); b.textContent = item.title;
  const s = document.createElement("span"); s.textContent = `${item.channel || "YouTube"}${item.itemCount ? ` · ${item.itemCount} faixas` : ""}`;
  const badge = document.createElement("em"); badge.textContent = "ABRIR PLAYLIST"; info.append(b, s, badge);
  const add = document.createElement("span"); add.className = "result-add playlist-play-icon"; add.innerHTML = icon("queue", 15); row.append(img, info, add);
  row.onclick = () => openPlaylistLibrary(item); appendToLastMusicSection(item).appendChild(row);
}
async function openPlaylistLibrary(item) {
  ensureAudio(); enableMusicAudio(); activePlaylistId = item.id; playlistModalTitle.textContent = item.title || "Playlist"; playlistLibraryStatus.textContent = "Carregando músicas…"; playlistLibraryList.innerHTML = ""; playlistHistoryList.innerHTML = ""; playlistModal.classList.remove("hidden"); renderPlaylistPicker();
  if (playlistLibrary[item.id]?.tracks?.length) {
    renderPlaylistLibrary(item.id);
    return;
  }
  try {
    ...
    const tracks = (data.results || []).filter(x => x.id && x.title).map(x => ({ ...x, playlistId: item.id, playlistTitle: item.title }));
    if (!tracks.length) { playlistLibraryStatus.textContent = "Essa playlist não tem faixas disponíveis."; return; }
    playlistLibrary[item.id] = { playlistId: item.id, playlistTitle: item.title, tracks, lastRefresh: Date.now() };
    renderPlaylistPicker();
    renderPlaylistLibrary(item.id);
    sendWS({ type: "playlist_adicionar", playlistId: item.id, playlistTitle: item.title, tracks });
  } catch { toast("Falha ao carregar a playlist.", true); }
}
function renderPlaylistLibrary(pid) {
  const lib = playlistLibrary[pid]; if (!lib) return;
  const playing = currentTrack?.id; const queued = new Set(queue.map(x => x.id)); const history = playlistHistory[pid] || [];
  playlistLibraryStatus.textContent = `${lib.tracks.length} músicas · ${history.length} tocadas · atualização manual`; playlistLibraryList.innerHTML = "";
  lib.tracks.forEach((t, i) => {
    const row = document.createElement("div"); row.className = "playlist-library-row";
    const img = document.createElement("img"); img.src = t.thumbnail || "";
    const info = document.createElement("div"); info.className = "playlist-library-info"; const title = document.createElement("strong"); title.textContent = t.title; const sub = document.createElement("span"); sub.textContent = t.channel || "YouTube"; info.append(title, sub);
    const status = document.createElement("em"); status.className = "playlist-track-status"; status.textContent = playing === t.id ? "TOCANDO" : queued.has(t.id) ? "NA FILA" : (history.some(x => x.id === t.id) ? "JÁ TOCOU" : "DISPONÍVEL");
    const actions = document.createElement("div"); actions.className = "playlist-library-actions";
    const add = document.createElement("button"); add.className = "ghost small"; add.innerHTML = `${icon("plus",13)} Fila`; add.onclick = () => { if (sendWS({ type: "playlist_fila_adicionar", playlistId: pid, videoId: t.id })) toast("Adicionada à fila"); };
    const play = document.createElement("button"); play.className = "primary small"; play.innerHTML = `${icon("play",13)} Tocar`; play.onclick = () => { ensureAudio(); enableMusicAudio(); if (sendWS({ type: "playlist_tocar", playlistId: pid, videoId: t.id })) toast("Tocando agora"); };
    actions.append(add, play); row.append(img, info, status, actions); playlistLibraryList.appendChild(row);
  });
  playlistHistoryList.innerHTML = "";
  [...history].reverse().forEach(t => { const row=document.createElement("div"); row.className="playlist-library-row history-row"; const img=document.createElement("img"); img.src=t.thumbnail||""; const info=document.createElement("div"); info.className="playlist-library-info"; const title=document.createElement("strong"); title.textContent=t.title; const sub=document.createElement("span"); sub.textContent="Tocada anteriormente"; info.append(title,sub); const actions=document.createElement("div"); actions.className="playlist-library-actions"; const play=document.createElement("button"); play.className="primary small"; play.innerHTML=`${icon("play",13)} Reproduzir`; play.onclick=()=>{ ensureAudio(); enableMusicAudio(); sendWS({type:"playlist_tocar",playlistId:pid,videoId:t.id}); }; const add=document.createElement("button"); add.className="ghost small"; add.innerHTML=`${icon("plus",13)} Fila`; add.onclick=()=>sendWS({type:"playlist_fila_adicionar",playlistId:pid,videoId:t.id}); actions.append(add,play); row.append(img,info,actions); playlistHistoryList.appendChild(row); });
  if (!history.length) playlistHistoryList.innerHTML='<div class="queue-empty">Ainda não há músicas tocadas desta playlist.</div>';
}
...
function renderQueue() {
  ...
  let draggedId = null;
  queue.forEach((m, i) => {
    const row = document.createElement("div"); row.className = "queue-row queue-row-playable"; row.title = "Clique para tocar agora ou arraste para mudar a ordem";
    row.draggable = true; row.dataset.videoId = m.id;
    row.addEventListener("dragstart", e => { draggedId = m.id; row.classList.add("dragging"); e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/plain", m.id); });
    row.addEventListener("dragend", () => { draggedId = null; row.classList.remove("dragging"); queueList.querySelectorAll(".queue-row").forEach(x => x.classList.remove("drag-over")); });
    row.addEventListener("dragover", e => { e.preventDefault(); if (draggedId && draggedId !== m.id) row.classList.add("drag-over"); e.dataTransfer.dropEffect = "move"; });
    row.addEventListener("dragleave", () => row.classList.remove("drag-over"));
    row.addEventListener("drop", e => {
      e.preventDefault(); row.classList.remove("drag-over");
      const from = draggedId || e.dataTransfer.getData("text/plain"); if (!from || from === m.id) return;
      const ids = queue.map(x => x.id); const fromIndex = ids.indexOf(from), toIndex = ids.indexOf(m.id);
      if (fromIndex < 0 || toIndex < 0) return;
      ids.splice(fromIndex, 1); ids.splice(toIndex, 0, from);
      if (sendWS({ type: "fila_reordenar", ids })) toast("Ordem da fila atualizada");
    });
    const n = document.createElement("b"); n.textContent = String(i + 1).padStart(2, "0");
    const img = document.createElement("img"); img.src = m.thumbnail || ""; img.alt = "";
    const info = document.createElement("div");
    const title = document.createElement("strong"); title.textContent = m.title;
    const channel = document.createElement("span"); channel.textContent = m.channel || "";
    info.append(title, channel);
    const play = document.createElement("button"); play.type = "button"; play.className = "queue-play-btn"; play.setAttribute("aria-label", "Tocar agora"); play.innerHTML = icon("play", 14);
    row.append(n, img, info, play); queueList.appendChild(row);
    const playNow = (ev) => { ev?.preventDefault(); ev?.stopPropagation(); ensureAudio(); enableMusicAudio(); if (sendWS({ type: "fila_tocar", videoId: m.id })) { toast("Tocando agora"); queueModal.classList.add("hidden"); } };
    row.addEventListener("click", playNow); play.addEventListener("click", playNow);
  });
}
function updateMiniQueue() {
  ...
  queue.slice(0, 3).forEach(m => {
    const d = document.createElement("div"); d.className = "mini-item mini-item-playable"; d.title = "Clique para tocar agora";
    const img = document.createElement("img"); img.src = m.thumbnail || ""; img.alt = "";
    const info = document.createElement("div"); info.className = "mini-info";
    const b = document.createElement("strong"); b.textContent = m.title;
    const s = document.createElement("span"); s.textContent = m.channel || "";
    info.append(b, s); d.append(img, info); miniQueue.appendChild(d);
    d.addEventListener("click", () => { ensureAudio(); enableMusicAudio(); if (sendWS({ type: "fila_tocar", videoId: m.id })) toast("Tocando agora"); });
  });
}
...
function togglePlayback() {
  ...
  if (currentTrack.paused) { if (sendWS({ type: "continuar_musica" })) syncLabel.textContent = "retomando…"; }
    ...
    let pos = targetPosition(currentTrack);
    try {
      if (yt && loadedVideoId === currentTrack.id) {
        const ytPos = Number(yt.getCurrentTime?.() || 0);
        // Nunca substitua uma posição válida por 0 só porque o player oculto
        // ainda não atualizou o currentTime no instante do clique.
        if (ytPos > 0 || pos <= 0) pos = ytPos;
      }
      yt?.pauseVideo?.();
    } catch {}
...
async function loadAdmin() {
  const [{ r: rr, data: rd }, { r: ur, data: ud }] = await Promise.all([api("/api/admin/rooms"), api("/api/admin/users")]);
  if (!rr.ok || !rd.ok || !ur.ok || !ud.ok) { toast((rd && rd.error) || (ud && ud.error) || "Acesso negado", true); return; }
  adminRooms.innerHTML = "";
  const roomsData = rd.rooms || [];
  const usersData = ud.users || [];
  for (const room of roomsData) {
    const drop = document.createElement("div"); drop.className = "admin-room admin-room-drop"; drop.dataset.room = room.nome;
    ...
    const s = document.createElement("span"); s.textContent = room.protegida ? "Sala privada · arraste usuários para liberar acesso" : "Sala pública"; info.append(b, s);
    ...
    drop.addEventListener("drop", async e => {
      ...
      const userId = e.dataTransfer.getData("application/x-chat-user") || e.dataTransfer.getData("text/plain");
      if (!userId) return;
      const x = await api("/api/admin/users/move", { method: "POST", body: JSON.stringify({ userId, roomName: room.nome }) });
      if (x.r.ok && x.data.ok) { toast(`Acesso à #${room.nome} liberado`); loadAdmin(); } else toast(x.data.error || "Não foi possível mover o usuário", true);
    });
    adminRooms.appendChild(drop);
  }
  const title = document.createElement("div"); title.className = "admin-section-title"; title.textContent = "Usuários · arraste para uma sala"; adminRooms.appendChild(title);
  const usersBox = document.createElement("div"); usersBox.className = "admin-users";
  for (const u of usersData) {
    const card = document.createElement("div"); card.className = "admin-user-card"; card.draggable = true; card.dataset.userId = u.id;
    card.innerHTML = `<span class="admin-user-avatar">${escapeHtml(initials(u.displayName || u.nome))}</span><span class="admin-user-copy"><strong>${escapeHtml(u.displayName || u.nome)}</strong><small>@${escapeHtml(u.nome)} · ${u.role === "admin" ? "admin" : "usuário"}</small></span><span class="drag-handle">${icon("menu", 15)}</span>`;
    card.addEventListener("dragstart", e => { e.dataTransfer.effectAllowed = "copy"; e.dataTransfer.setData("application/x-chat-user", u.id); e.dataTransfer.setData("text/plain", u.id); card.classList.add("dragging-user"); });
    card.addEventListener("dragend", () => card.classList.remove("dragging-user"));
    usersBox.appendChild(card);
  }
  adminRooms.appendChild(usersBox);
}
