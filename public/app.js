...
let meuId = localStorage.getItem("usuarioId");
if (!meuId) {
  meuId = crypto.randomUUID();
  localStorage.setItem("usuarioId", meuId);
}
...
function carregarSalas() {
  ...
  return [{ nome: "geral", emoji: "#", senha: "", configurarSenha: false }];
}
...
function renderizarSalas() {
  ...
  for (const sala of salas) {
    ...
    item.textContent = `${sala.senha ? "🔒" : sala.emoji} ${sala.nome}`;
...
btnNovaSala.addEventListener("click", () => {
  inputSala.value = "";
  inputSalaSenha.value = "";
  checkSalaSenha.checked = false;
  inputSalaSenha.classList.add("escondido");
  erroSala.textContent = "";
  modalSala.classList.remove("escondido");
  inputSala.focus();
});

checkSalaSenha.addEventListener("change", () => {
  inputSalaSenha.classList.toggle("escondido", !checkSalaSenha.checked);
  if (checkSalaSenha.checked) inputSalaSenha.focus();
});
...
btnCriarSala.addEventListener("click", () => {
  ...
  if (salas.some((s) => s.nome === nome)) {
    erroSala.textContent = "Já existe uma sala com esse nome neste navegador.";
    return;
  }
  if (checkSalaSenha.checked && senha.length < 4) {
    erroSala.textContent = "Use uma senha com pelo menos 4 caracteres.";
    return;
  }
  salas.push({
    nome,
    emoji: emojiEscolhido,
    senha: checkSalaSenha.checked ? senha : "",
    configurarSenha: checkSalaSenha.checked
  });
  salvarSalas();
...
btnCancelarSenha.addEventListener("click", () => {
  modalSenhaSala.classList.add("escondido");
  if (socket) {
    socket.onclose = null;
    socket.close();
  }
...
btnEntrarSenha.addEventListener("click", () => {
  const senha = inputEntrarSenha.value;
  if (!senha) {
    erroSenhaSala.textContent = "Digite a senha.";
    return;
  }
  erroSenhaSala.textContent = "";
  const sala = salaAtualObjeto();
  sala.senha = senha;
  sala.configurarSenha = false;
  salvarSalas();
  renderizarSalas();
  modalSenhaSala.classList.add("escondido");
  enviarEntradaNaSala(senha);
});

inputEntrarSenha.addEventListener("keydown", (e) => {
  if (e.key === "Enter") btnEntrarSenha.click();
});
...
function conectarWebSocket(nomeSala) {
  ...
  socket.addEventListener("open", () => {
    presencaSala.textContent = "autenticando...";
...
function salaAtualObjeto() {
  return salas.find((s) => s.nome === salaAtual) || {
    nome: salaAtual,
    emoji: "#",
    senha: "",
    configurarSenha: false
  };
}

function enviarEntradaNaSala(senhaOverride = null) {
  const sala = salaAtualObjeto();
  const senha = senhaOverride !== null ? senhaOverride : (sala.senha || "");
  enviarWS({
    type: "entrar",
    nome: meuNome,
    usuarioId: meuId,
    senha,
    configurarSenha: Boolean(sala.configurarSenha && senha),
  });
}
...
function tratarEventoServidor(dados) {
  switch (dados.type) {
    case "autenticacao_necessaria":
      ...
      if (dados.protegida) {
        erroSenhaSala.textContent = "";
        inputEntrarSenha.value = "";
        modalSenhaSala.classList.remove("escondido");
        inputEntrarSenha.focus();
      } else {
        enviarEntradaNaSala();
      }
      break;
    ...
    case "erro":
      if (dados.code === "SENHA_INCORRETA" || dados.code === "SENHA_NECESSARIA") {
        conexaoAutenticada = false;
        erroSenhaSala.textContent = dados.message;
        modalSenhaSala.classList.remove("escondido");
        inputEntrarSenha.focus();
      } else {
...
btnPausarMusica.addEventListener("click", () => {
  ...
  enviarWS({ type: tocandoAgoraAtual.paused ? "continuar_musica" : "pausar_musica" });
});
...
btnAbrirFila.addEventListener("click", () => {
  modalFila.classList.remove("escondido");
...
function renderizarFila(fila) {
  ...
  if (!filaAtual.length) {
    ...
    vazio.textContent = "A fila está vazia.";
  ...
  filaAtual.forEach((musica, index) => {
    ...
    const canal = document.createElement("div");
    canal.className = "fila-canal";
    canal.textContent = musica.channel || "";

    info.append(titulo, canal);
    item.append(numero, img, info);
...
function atualizarPainelRadio(tocandoAgora, fila) {
  ...
  if (!tocandoAgora) {
    ...
    capaRadio.innerHTML = "🎵";
    btnPausarMusica.classList.add("escondido");
    btnPularMusica.classList.add("escondido");
    filaPreview.textContent = "Fila vazia";
    pararPlayer();
    return;
  }
  ...
  btnPausarMusica.classList.remove("escondido");
  ...
  filaPreview.textContent = filaAtual.length > 0
    ? `${filaAtual.length} ${filaAtual.length === 1 ? "música na fila" : "músicas na fila"}`
    : "Fila vazia";
...
function reproduzir(musica) {
  let segundos = 0;
  if (musica.paused && musica.pausedAt) {
    segundos = Math.max(0, Math.floor((musica.pausedAt - musica.startedAt) / 1000));
  } else {
    segundos = Math.max(0, Math.floor((Date.now() - musica.startedAt) / 1000));
  }
  ...
  if (!audioLiberado) ytPlayer.mute();
  else ytPlayer.unMute();

  if (musica.paused) {
    setTimeout(() => {
      if (ytPlayer && tocandoAgoraAtual && tocandoAgoraAtual.id === musica.id && tocandoAgoraAtual.paused) {
        ytPlayer.pauseVideo();
      }
    }, 350);
  }
  ...
  ytPlayer = new YT.Player("player-youtube", {
    ...
    events: {
      ...
      onStateChange: (evento) => {
        ...
        if (evento.data === YT.PlayerState.PAUSED && !audioLiberado && tocandoAgoraAtual && !tocandoAgoraAtual.paused) {
