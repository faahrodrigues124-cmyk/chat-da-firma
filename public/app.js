...
const playlistAutoQueue = $("playlist-auto-queue");
...
function handleServer(d) {
  ...
  if (d.type === "estado_inicial") {
    ...
    queue = d.fila || []; playlistLibrary = d.playlistLibrary || {}; playlistHistory = d.playlistHistory || {}; applyRadioState(d.tocandoAgora, d.fila, true); if (Array.isArray(d.playlistsAuto) && d.playlistsAuto.length) toast(`Playlist automática ativa: ${d.playlistsAuto[0].playlistTitle || "playlist"}`); scrollBottom(true);
  ...
  if (d.type === "playlist_atualizada") { if (d.playlistLibrary) playlistLibrary = d.playlistLibrary; if (d.playlistHistory) playlistHistory = d.playlistHistory; queue = queue; updateMiniQueue(); if (activePlaylistId) renderPlaylistLibrary(activePlaylistId); if (Array.isArray(d.playlists) && d.playlists.length) toast(`Playlist atualizada · ${d.playlists[0].playlistTitle || "novas faixas"}`); return; }
...
async function openPlaylistLibrary(item) {
  ensureAudio(); enableMusicAudio(); activePlaylistId = item.id; playlistModalTitle.textContent = item.title || "Playlist"; playlistLibraryStatus.textContent = "Carregando músicas…"; playlistLibraryList.innerHTML = ""; playlistHistoryList.innerHTML = ""; playlistModal.classList.remove("hidden");
  try {
    ...
    sendWS({ type: "playlist_adicionar", playlistId: item.id, playlistTitle: item.title, tracks, autoUpdate: playlistAutoUpdate?.checked !== false, autoQueue: playlistAutoQueue?.checked !== false });
...
function renderPlaylistLibrary(pid) {
  ...
  playlistLibraryStatus.textContent = `${lib.tracks.length} músicas · ${history.length} tocadas · atualização automática ativa`; playlistLibraryList.innerHTML = "";
