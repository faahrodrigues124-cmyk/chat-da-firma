...
const iconPaths = {
  ...
  gif: '<rect x="3" y="5" width="18" height="14" rx="2"/><path d="M8 10v4M8 10h3M8 12h2M13 10v4M16 10h3M16 12h2"/>',
...
function buildVolumeMixer() {
  ...
  volumeMuteButton.className = "icon-btn";
  volumeMuteButton.style.cssText = "width:30px;height:30px;display:grid;place-items:center;padding:0;flex:none";
  ...
  volumeSlider.style.cssText = "width:100px;accent-color:#9b7cff;cursor:pointer";
  volumeSlider.setAttribute("aria-label", "Volume da música");

  volumeValue = document.createElement("span");
  volumeValue.style.cssText = "min-width:34px;text-align:right;font-size:11px;font-variant-numeric:tabular-nums;opacity:.72";
  ...
  // Coloca o mixer próximo aos controles do player sem substituir
  // o botão de notificações existente.
  if (sound?.parentElement === anchor) {
    anchor.insertBefore(wrap, sound);
  } else {
...
function buildMediaControls() {
  if ($("media-tools")) return;
  const host = messageInput?.parentElement;
  if (!host) return;

  const tools = document.createElement("div");
  tools.id = "media-tools";
  tools.style.cssText = "display:flex;align-items:center;gap:6px;margin:0 0 7px 2px;";

  mediaFileInput = document.createElement("input");
  mediaFileInput.type = "file";
  mediaFileInput.accept = "image/*";
  mediaFileInput.hidden = true;
  mediaFileInput.onchange = () => { const f = mediaFileInput.files?.[0]; if (f) sendMediaFile(f, false); mediaFileInput.value = ""; };

  gifFileInput = document.createElement("input");
  gifFileInput.type = "file";
  gifFileInput.accept = "image/gif,.gif";
  gifFileInput.hidden = true;
  gifFileInput.onchange = () => { const f = gifFileInput.files?.[0]; if (f) sendMediaFile(f, true); gifFileInput.value = ""; };

  const photo = document.createElement("button");
  photo.type = "button"; photo.className = "media-action";
  photo.innerHTML = `${icon("image", 15)}<span>Foto</span>`;
  photo.title = "Enviar foto";
  photo.onclick = () => mediaFileInput.click();

  const gif = document.createElement("button");
  gif.type = "button"; gif.className = "media-action";
  gif.innerHTML = `${icon("gif", 15)}<span>GIF</span>`;
  gif.title = "Enviar GIF";
  gif.onclick = () => gifFileInput.click();

  const hint = document.createElement("span");
  hint.className = "media-hint";
  hint.textContent = "Fotos até 500 KB · GIF até 800 KB";

  tools.append(photo, gif, hint, mediaFileInput, gifFileInput);
  host.insertBefore(tools, messageInput);

  const style = document.createElement("style");
  style.textContent = `
    .media-action{display:inline-flex;align-items:center;gap:6px;height:30px;padding:0 10px;border:1px solid rgba(255,255,255,.09);border-radius:9px;background:rgba(255,255,255,.045);color:inherit;cursor:pointer;font:inherit;font-size:12px;transition:.18s}
    .media-action:hover{background:rgba(255,255,255,.09);transform:translateY(-1px)}
    .media-hint{font-size:10px;opacity:.42;margin-left:2px}
    .message-media{display:block;max-width:min(420px,75vw);max-height:420px;margin-top:8px;border-radius:12px;object-fit:contain;background:rgba(0,0,0,.18)}
    .message-media.gif{image-rendering:auto}
    .message-caption{margin-top:7px;white-space:pre-wrap;overflow-wrap:anywhere}
  `;
...
async function compressPhoto(file) {
  ...
  while (out.length > 500000 && quality > 0.48) {
    quality -= 0.08;
  ...
  if (out.length > 500000) throw new Error("A foto continua grande demais depois da compressão.");
  return { kind: "image", mime: "image/webp", data: out };
}

async function sendMediaFile(file, forceGif) {
  if (!authenticated) return toast("A sala ainda está conectando.", true);
  if (!file.type.startsWith("image/")) return toast("Escolha uma imagem ou GIF.", true);
  const isGif = forceGif || file.type === "image/gif";
  const maxBytes = isGif ? 600000 : 500000;
  if (file.size > maxBytes) return toast(`${isGif ? "GIF" : "Imagem"} muito grande. Limite: ${Math.round(maxBytes / 1000)} KB.`, true);

  try {
    toast("Preparando mídia…");
    const media = isGif ? { kind: "gif", mime: "image/gif", data: await readFileAsDataURL(file) } : await compressPhoto(file);
    if (media.data.length > 900000) throw new Error("Arquivo grande demais para a sala.");
    if (sendWS({ type: "mensagem", text: "", media })) toast(isGif ? "GIF enviado" : "Foto enviada");
  } catch (e) {
    toast(e?.message || "Não foi possível enviar a mídia.", true);
  }
}
...
async function boot() {
  ...
  messageInput.addEventListener("paste", e => {
    ...
    if (file) { e.preventDefault(); sendMediaFile(file, file.type === "image/gif"); }
  });
...
function renderMessage(m) {
  const row = document.createElement("article"); row.className = `message-row${m.autorId === user.id ? " mine" : ""}`; row.dataset.id = m.id;
  ...
  if (m.media?.data) {
    ...
    img.className = `message-media${m.media.kind === "gif" ? " gif" : ""}`;
    img.src = m.media.data;
    img.alt = m.media.kind === "gif" ? "GIF enviado" : "Foto enviada";
    img.loading = "lazy";
    bubble.appendChild(img);
  }
  if (m.text) {
    const caption = document.createElement("div"); caption.className = m.media ? "message-caption" : ""; caption.textContent = m.text; bubble.appendChild(caption);
  }
...
window.onYouTubeIframeAPIReady = function () {
  if (yt) return;
  window.__salaPlayer = new YT.Player("youtube-player", { height: "1", width: "1", playerVars: { autoplay: 1, playsinline: 1, controls: 0, rel: 0, modestbranding: 1, enablejsapi: 1 }, events: {
    onReady: onYTReady,
    onStateChange: e => {
