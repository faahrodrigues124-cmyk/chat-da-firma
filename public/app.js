...
function salvarSessao() {
  localStorage.setItem(SESSION_KEY, JSON.stringify({ token: sessionToken, id: meuId, nome: meuNome }));
}
function carregarSessao() {
  try {
    const s = JSON.parse(localStorage.getItem(SESSION_KEY));
    if (s?.token && s?.id && s?.nome) { sessionToken = s.token; meuId = s.id; meuNome = s.nome; return true; }
  } catch {}
  return false;
}
function limparSessao() { localStorage.removeItem(SESSION_KEY); sessionToken = ""; meuId = ""; meuNome = ""; }
...
async function autenticar() {
  ...
  try {
    ...
    sessionToken = dados.token; meuId = dados.user.id; meuNome = dados.user.nome; salvarSessao();
...
function conectarWebSocket(nomeSala) {
  ...
  socket = new WebSocket(`${protocolo}//${location.host}/ws?room=${encodeURIComponent(nomeSala)}`);
