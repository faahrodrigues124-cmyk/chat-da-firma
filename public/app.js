...
function sendWS(obj) {
  if (socket?.readyState !== WebSocket.OPEN || !authenticated) { toast("A sala ainda não está pronta.", true); return false; }
  try { socket.send(JSON.stringify(obj)); return true; } catch { toast("Não foi possível enviar agora.", true); return false; }
}
