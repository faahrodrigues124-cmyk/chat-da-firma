...
export class AppData {
  constructor(state) {
    ...
    this.state.blockConcurrencyWhile(async () => {
      ...
      if (usuarios.length && !usuarios.some((u) => u.role === "admin")) {
        usuarios[0].role = "admin";
        this.users[usuarios[0].nome.toLowerCase()] = usuarios[0];
      }
      await this.persist();
    });
  }
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/internal/register" && request.method === "POST") {
      ...
      this.users[key] = { id, nome: b.nome, senhaHash: b.senhaHash, role: Object.keys(this.users).length === 0 ? "admin" : "user" };
