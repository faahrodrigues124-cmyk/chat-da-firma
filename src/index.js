...
export class ChatRoom {
  ...
  async mensagem(socket, raw) {
    ...
    if (d.type === "mensagem") {
      ...
      if (text.length > MAX_MENSAGEM) return this.enviar(socket, { type: "erro", message: `Mensagem muito longa (máx. ${MAX_MENSAGEM}).` });

      let media = null;
      if (d.media && typeof d.media === "object") {
        const kind = d.media.kind === "gif" ? "gif" : d.media.kind === "image" ? "image" : "";
        const data = String(d.media.data || "");
        const mime = String(d.media.mime || "");
        const allowed = kind && ["image/gif", "image/jpeg", "image/png", "image/webp"].includes(mime);
        const prefixOk = data.startsWith(`data:${mime};base64,`);
        const limit = kind === "gif" ? 650000 : 550000;
        if (!allowed || !prefixOk || data.length > limit || data.length > MAX_MEDIA_CHARS) {
          return this.enviar(socket, { type: "erro", message: "Imagem/GIF inválido ou grande demais." });
        }
        media = { kind, mime, data };
      }

      if (!text && !media) return;
      const msg = { id: crypto.randomUUID(), autorId: s.usuarioId, nome: s.nome, text, media, ts: Date.now(), reacoes: {} };
      this.mensagens.push(msg);
      if (this.mensagens.length > MAX_HISTORICO) this.mensagens.shift();
      await this.persistir();
      this.transmitir({ type: "mensagem", mensagem: msg });
      return;
    }
    if (d.type === "reacao") {
