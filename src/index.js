...
export class ChatRoom {
    ...
    for (const source of this.playlistAuto) {
      try {
        ...
        if (novas.length && (espaco > 0 || !this.tocandoAgora)) {
          ...
          if (!this.tocandoAgora && adicionadas.length) {
            const primeira = adicionadas.shift();
            this.tocandoAgora = { ...primeira, startedAt: Date.now(), position: 0, paused: false };
            mudou = true;
          }
  ...
  async alarm() {
    ...
    const mudou = await this.atualizarPlaylistsAutomaticas();
    await this.persistir();
    if (mudou) this.transmitir({ type: "tocando_agora", tocandoAgora: this.tocandoAgora, fila: this.fila });
    this.transmitir({ type: "playlist_atualizada", playlists: this.playlistAuto });
