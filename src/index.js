...
async function chamarYoutube(caminho, parametros, env) {
  ...
  for (const chave of chaves) {
    ...
    // O YouTube pode sinalizar esgotamento/limitação de cota com razões diferentes
    // (quotaExceeded, dailyLimitExceeded, rateLimitExceeded, userRateLimitExceeded)
    // e também pode usar 429/RESOURCE_EXHAUSTED. Todos esses casos devem fazer o
    // Worker experimentar a próxima chave antes de devolver erro ao navegador.
    ...
    // Cota/limite da chave atual: tenta imediatamente a próxima chave.
  }
...
export class ChatRoom {
  constructor(state, env) {
    this.state = state; this.env = env; this.sessoes = new Map(); this.mensagens = []; this.fila = []; this.tocandoAgora = null; this.senhaHash = ""; this.ownerId = ""; this.radioOn = false; this.ultimoAvancoEm = 0; this.playlistAuto = []; this.tocadas = [];
    this.state.blockConcurrencyWhile(async () => {
      const s = await this.state.storage.get(["mensagens", "fila", "tocandoAgora", "senhaHash", "ownerId", "radioOn", "playlistAuto", "tocadas"]);
      this.mensagens = s.get("mensagens") || []; this.fila = s.get("fila") || []; this.tocandoAgora = s.get("tocandoAgora") || null; this.senhaHash = s.get("senhaHash") || ""; this.ownerId = s.get("ownerId") || ""; this.radioOn = Boolean(s.get("radioOn")); this.playlistAuto = s.get("playlistAuto") || []; this.tocadas = s.get("tocadas") || [];
    });
  }
  async persistir() { await this.state.storage.put({ mensagens: this.mensagens, fila: this.fila, tocandoAgora: this.tocandoAgora, senhaHash: this.senhaHash, ownerId: this.ownerId, radioOn: this.radioOn, playlistAuto: this.playlistAuto, tocadas: this.tocadas }); }
  async alarm() {
    if (!this.playlistAuto.length) return;
    let mudou = false;
    for (const source of this.playlistAuto) {
      try {
        ...
        if (espaco > 0 && novas.length) {
          this.fila.push(...novas.slice(0, espaco).map(x => ({ ...x, requestedBy: source.requestedBy || "Playlist automática", playlistId: source.playlistId, playlistTitle: source.playlistTitle, playlistAuto: true })));
          mudou = true;
        }
        source.lastRefresh = Date.now();
      } catch {}
    }
    if (mudou) { await this.persistir(); this.transmitir({ type: "tocando_agora", tocandoAgora: this.tocandoAgora, fila: this.fila }); this.transmitir({ type: "playlist_atualizada", playlists: this.playlistAuto }); }
    else await this.persistir();
    if (this.playlistAuto.length) await this.state.storage.setAlarm(Date.now() + 60000);
  }
  ...
  async mensagem(socket, raw) {
    ...
    if (d.type === "playlist_adicionar") {
      ...
      const playlistId = String(d.playlistId || "");
      const playlistTitle = String(d.playlistTitle || "Playlist");
      const autoUpdate = d.autoUpdate !== false;
      const existentes = new Set([this.tocandoAgora?.id, ...this.fila.map(x => x.id), ...this.tocadas].filter(Boolean));
      const novas = tracks.filter(x => !existentes.has(x.id));
      if (playlistId) {
        if (autoUpdate) {
          const idx = this.playlistAuto.findIndex(x => x.playlistId === playlistId);
          const source = { playlistId, playlistTitle, requestedBy: s.nome, lastRefresh: Date.now() };
          if (idx >= 0) this.playlistAuto[idx] = { ...this.playlistAuto[idx], ...source };
          else this.playlistAuto.push(source);
          await this.state.storage.setAlarm(Date.now() + 60000);
        } else {
          this.playlistAuto = this.playlistAuto.filter(x => x.playlistId !== playlistId);
          if (!this.playlistAuto.length) try { await this.state.storage.deleteAlarm(); } catch {}
        }
      }
      ...
      if (!this.tocandoAgora) {
        ...
        this.tocandoAgora = { ...primeira, startedAt: agora, position: 0, paused: false, requestedBy: s.nome, playlistId, playlistTitle };
      }
      const espaco = Math.max(0, MAX_FILA - this.fila.length);
      this.fila.push(...novas.slice(0, espaco).map(x => ({ ...x, requestedBy: s.nome, playlistId, playlistTitle })));
      ...
      const musicMsg = { id: crypto.randomUUID(), autorId: s.usuarioId, nome: s.nome, text: "", ts: agora, reacoes: {}, tipo: "musica", music: {
        ...
        playlist: true, playlistTitle, playlistId
      } };
    ...
    if (d.type === "playlist_auto_toggle") {
      ...
      if (d.enabled) {
        const source = { playlistId, playlistTitle: String(d.playlistTitle || "Playlist"), requestedBy: s.nome, lastRefresh: Date.now() };
        const idx = this.playlistAuto.findIndex(x => x.playlistId === playlistId);
        if (idx >= 0) this.playlistAuto[idx] = { ...this.playlistAuto[idx], ...source }; else this.playlistAuto.push(source);
        await this.state.storage.setAlarm(Date.now() + 1000);
      } else {
        this.playlistAuto = this.playlistAuto.filter(x => x.playlistId !== playlistId);
        if (!this.playlistAuto.length) try { await this.state.storage.deleteAlarm(); } catch {}
      }
      await this.persistir(); this.transmitir({ type: "playlist_atualizada", playlists: this.playlistAuto }); return;
    }
