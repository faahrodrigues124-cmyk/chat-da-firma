...
async function buscarMusica(url, env) {
  ...
  const emCache = await cacheYoutubeGet(env, chaveCache, 6 * 60 * 60 * 1000); // 6h
  if (emCache) return json({ ok: true, results: emCache });

  const resultado = await chamarYoutube("search", {
    part: "snippet", type: "video", maxResults: "10", videoEmbeddable: "true", q: termo,
  }, env);
  ...
  const results = (resultado.data.items || []).filter((x) => x.id?.videoId && x.snippet).map((x) => ({
    id: x.id.videoId,
...
export class ChatRoom {
  constructor(state, env) {
    this.state = state; this.env = env; this.sessoes = new Map(); this.mensagens = []; this.fila = []; this.tocandoAgora = null; this.senhaHash = ""; this.ownerId = ""; this.radioOn = false; this.ultimoAvancoEm = 0; this.musicHistory = [];
    this.state.blockConcurrencyWhile(async () => {
      const s = await this.state.storage.get(["mensagens", "fila", "tocandoAgora", "senhaHash", "ownerId", "radioOn", "musicHistory"]);
      this.mensagens = s.get("mensagens") || []; this.fila = s.get("fila") || []; this.tocandoAgora = s.get("tocandoAgora") || null; this.senhaHash = s.get("senhaHash") || ""; this.ownerId = s.get("ownerId") || ""; this.radioOn = Boolean(s.get("radioOn")); this.musicHistory = s.get("musicHistory") || [];
    });
  }
  async persistir() { await this.state.storage.put({ mensagens: this.mensagens, fila: this.fila, tocandoAgora: this.tocandoAgora, senhaHash: this.senhaHash, ownerId: this.ownerId, radioOn: this.radioOn, musicHistory: this.musicHistory }); }
  registrarExecucao(track) {
    if (!track?.id) return;
    this.musicHistory.push({
      id: track.id, title: track.title || "Música", thumbnail: track.thumbnail || "",
      addedById: track.addedById || "", addedByName: track.addedByName || "",
      ts: Date.now(), recommendation: Boolean(track.recommendation)
    });
    const limite = Date.now() - 30 * 86400000;
    this.musicHistory = this.musicHistory.filter((x) => x.ts >= limite).slice(-500);
  }
  async enviarRecapSemanal(socket) {
    const limite = Date.now() - 7 * 86400000;
    const plays = this.musicHistory.filter((x) => x.ts >= limite);
    const msgs = this.mensagens.filter((x) => x.ts >= limite).length;
    const users = {}; const tracks = {};
    for (const p of plays) {
      if (p.addedByName) users[p.addedByName] = (users[p.addedByName] || 0) + 1;
      const k = p.id;
      if (!tracks[k]) tracks[k] = { id: k, title: p.title, thumbnail: p.thumbnail, count: 0 };
      tracks[k].count++;
    }
    const topUsers = Object.entries(users).map(([name,count]) => ({ name, count })).sort((a,b)=>b.count-a.count).slice(0,5);
    const topTracks = Object.values(tracks).sort((a,b)=>b.count-a.count).slice(0,5);
    this.enviar(socket, { type: "weekly_recap", stats: { plays: plays.length, uniqueTracks: Object.keys(tracks).length, messages: msgs }, topUsers, topTracks });
  }
  async preencherRecomendacoes() {
    ...
    for (const item of itens) {
      ...
      this.fila.push({ ...item, recommendation: true });
  ...
  async mensagem(socket, raw) {
    ...
    if (d.type === "weekly_recap") { await this.enviarRecapSemanal(socket); return; }
    ...
    if (d.type === "fila_adicionar") {
      ...
      const music = { id: d.id, title: d.title, channel: d.channel || "", thumbnail: d.thumbnail || "", addedById: s.usuarioId, addedByName: s.nome };
      if (!this.tocandoAgora) { this.tocandoAgora = { ...music, startedAt: Date.now(), position: 0, paused: false }; this.registrarExecucao(this.tocandoAgora); }
    ...
    if (d.type === "proxima_musica") {
      ...
      if (this.tocandoAgora) this.registrarExecucao(this.tocandoAgora);
      if (this.radioOn && this.tocandoAgora) await this.preencherRecomendacoes();
      await this.persistir(); this.transmitir({ type: "tocando_agora", tocandoAgora: this.tocandoAgora, fila: this.fila }); return;
    }
  }
