...
export class ChatRoom {
  ...
  async preencherRecomendacoes() {
    if (!this.radioOn || !this.tocandoAgora || !this.env.CHAVE_API_DO_YOUTUBE) return;
    const faltam = Math.max(0, 3 - this.fila.length);
    if (!faltam) return;
    const api = new URL("https://www.googleapis.com/youtube/v3/search");
    api.searchParams.set("part", "snippet");
    api.searchParams.set("type", "video");
    api.searchParams.set("maxResults", String(Math.min(8, Math.max(5, faltam + 3))));
    api.searchParams.set("videoEmbeddable", "true");
    api.searchParams.set("relatedToVideoId", this.tocandoAgora.id);
    api.searchParams.set("key", this.env.CHAVE_API_DO_YOUTUBE);
    try {
      ...
      for (const x of (data.items || [])) {
        ...
        if (!id || existentes.has(id) || !x.snippet) continue;
        this.fila.push({ id, title: x.snippet.title, channel: x.snippet.channelTitle || "", thumbnail: x.snippet.thumbnails?.high?.url || x.snippet.thumbnails?.medium?.url || x.snippet.thumbnails?.default?.url || "", recommendation: true });
        existentes.add(id);
