...
export class AppData {
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/internal/register" && request.method === "POST") {
      ...
      this.users[key] = { id, nome: b.nome, senhaHash: b.senhaHash };
      const token = gerarToken();
      this.sessions[token] = { userId: id, expiresAt: Date.now() + SESSION_DIAS * 86400000 };
      await this.persist();
      return json({ ok: true, token, user: { id, nome: b.nome } });
    }
    if (url.pathname === "/internal/login" && request.method === "POST") {
      ...
      await this.persist();
      return json({ ok: true, token, user: { id: user.id, nome: user.nome } });
    }
    if (url.pathname.startsWith("/internal/session/")) {
      ...
      return json({ id: user.id, nome: user.nome });
    }
