...
export class ChatRoom {
  ...
  async mensagem(socket, raw) {
    ...
    if (d.type === "playlist_adicionar") {
      ...
      if (playlistId && autoUpdate) {
        const idx = this.playlistAuto.findIndex(x => x.playlistId === playlistId);
        const source = { playlistId, playlistTitle, requestedBy: s.nome, lastRefresh: Date.now(), autoQueue: d.autoQueue !== false };
        if (idx >= 0) this.playlistAuto[idx] = { ...this.playlistAuto[idx], ...source }; else this.playlistAuto.push(source);
        await this.state.storage.setAlarm(Date.now() + 60000);
      } else if (playlistId) { this.playlistAuto = this.playlistAuto.filter(x => x.playlistId !== playlistId); if (!this.playlistAuto.length) try { await this.state.storage.deleteAlarm(); } catch {} }
      await this.persistir();
    ...
    if (d.type === "playlist_refresh_now") {
      ...
      const source = this.playlistAuto.find(x => x.playlistId === pid);
      if (!source) return this.enviar(socket, { type: "erro", message: "Essa playlist não está em atualização automática." });
      try {
        const tracks = await this.carregarPlaylistAtual(pid);
        const antiga = this.playlistLibrary[pid]?.tracks || []; const mapa = new Map(antiga.map(x => [String(x.id), x]));
        for (const t of tracks) mapa.set(String(t.id), { ...t, playlistId: pid, playlistTitle: source.playlistTitle });
        this.playlistLibrary[pid] = { playlistId: pid, playlistTitle: source.playlistTitle, tracks: [...mapa.values()], lastRefresh: Date.now() };
        if (source.autoQueue !== false) {
          const existentes = new Set([this.tocandoAgora?.id, ...this.fila.map(x => x.id), ...this.tocadas].filter(Boolean));
          const novas = tracks.filter(x => x.id && !existentes.has(x.id)); const espaco = Math.max(0, MAX_FILA - this.fila.length);
          const limite = Math.max(0, espaco) + (!this.tocandoAgora ? 1 : 0);
          const add = novas.slice(0, limite).map(x => ({ ...x, requestedBy: s.nome, playlistId: pid, playlistTitle: source.playlistTitle, playlistAuto: true }));
          if (!this.tocandoAgora && add.length) { const first = add.shift(); this.tocandoAgora = { ...first, startedAt: Date.now(), position: 0, paused: false }; }
          this.fila.push(...add);
        }
        source.lastRefresh = Date.now(); await this.persistir();
        this.transmitir({ type: "playlist_atualizada", playlists: this.playlistAuto, playlistLibrary: this.playlistLibrary, playlistHistory: this.playlistHistory, tocandoAgora: this.tocandoAgora, fila: this.fila });
