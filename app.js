...
async function boot() {
  ...
  radioIcon.innerHTML = icon("radio", 14);
  updateRadioToggle();
  updateSoundButton();
  updateVolumeUI();
  ...
  radioToggle.onclick = () => { if (sendWS({ type: "radio_toggle", enabled: !radioOn })) radioOn = !radioOn, updateRadioToggle(); };
...
function handleServer(d) {
  ...
  if (d.type === "estado_inicial") {
    ...
    queue = d.fila || []; radioOn = Boolean(d.radioOn); updateRadioToggle(); applyRadioState(d.tocandoAgora, d.fila, true); scrollBottom();
  ...
  if (d.type === "radio_estado") { radioOn = Boolean(d.radioOn); queue = d.fila || queue; updateRadioToggle(); updateMiniQueue(); return; }
  if (d.type === "weekly_recap") { renderRecap(d); return; }
...
function renderMessage(m) {
  ...
  if (m.music) {
    const card = document.createElement("div"); card.className = "music-chat-card";
    const img = document.createElement("img"); img.src = m.music.thumbnail || ""; img.alt = "";
    const info = document.createElement("div"); info.className = "music-chat-info";
    const tag = document.createElement("span"); tag.className = "music-chat-tag"; tag.textContent = "TOCOU";
    const title = document.createElement("strong"); title.textContent = m.music.title || "Música";
    const channel = document.createElement("span"); channel.textContent = m.music.channel || "YouTube";
    info.append(tag, title, channel); card.append(img, info);
    if (m.music.id) card.onclick = () => { if (authenticated) sendWS({ type: "fila_adicionar", id:m.music.id, title:m.music.title, channel:m.music.channel, thumbnail:m.music.thumbnail }); };
    bubble.appendChild(card);
  } else if (m.mediaType === "gif" || m.mediaType === "image") {
...
function renderGifResult(item) {
  const b = document.createElement("button"); b.className = "gif-card"; b.type = "button"; b.title = "Enviar GIF";
...
function processImageFile(file) {
  ...
  if (file.size > 12 * 1024 * 1024) { toast("A imagem é muito grande.", true); return false; }
  compressImage(file).then(dataUrl => {
    if (dataUrl.length > 500000) return toast("Não consegui reduzir essa imagem o suficiente.", true);
    if (sendMedia(dataUrl, "image", file.name || "Imagem colada")) toast("Imagem enviada");
  }).catch(() => toast("Não foi possível processar a imagem.", true));
  return true;
}
...
function setupImagePasteAndDrop() {
  // --- COLAR IMAGEM ---
  // Capturamos no window em fase de capture para que o textarea, extensões
  // ou outros handlers da página não impeçam o envio.
  const handlePasteImage = e => {
    ...
    // Caminho 1: ClipboardItem (Chrome/Edge/Firefox)
    ...
    // Caminho 2: files do clipboard (alguns navegadores/Windows)
    if (!file && clipboard.files?.length) {
      file = Array.from(clipboard.files).find(f =>
        /^image\//i.test(f.type)
      ) || null;
    }
  ...
  window.addEventListener("paste", handlePasteImage, true);

  // --- ARRASTAR E SOLTAR ---
  // O browser precisa receber preventDefault no dragover para permitir o drop.
  // Fazemos isso globalmente e só processamos quando houver um arquivo de imagem.
  let draggingImage = false;

  window.addEventListener("dragenter", e => {
    const files = e.dataTransfer?.types || [];
    if (!Array.from(files).includes("Files")) return;

    draggingImage = true;
    document.body.classList.add("is-dragging-image");

    const target = e.target?.closest?.(".messages, .composer");
    if (target) target.classList.add("drop-image-active");
  }, true);
  ...
  window.addEventListener("dragleave", e => {
    // Só encerra o estado quando realmente sair da janela.
    if (e.clientX <= 0 || e.clientY <= 0 ||
        e.clientX >= window.innerWidth || e.clientY >= window.innerHeight) {
      draggingImage = false;
      document.body.classList.remove("is-dragging-image");
      document.querySelectorAll(".drop-image-active")
        .forEach(el => el.classList.remove("drop-image-active"));
    }
  }, true);
  ...
  window.addEventListener("drop", e => {
    ...
    // Se não for um arquivo de imagem, não interferimos no comportamento normal.
    ...
    // Envia a primeira imagem; evita mandar várias sem querer.
    processImageFile(images[0]);
  }, true);

  // Feedback visual local no chat/compositor.
  const dropTargets = [messages, document.querySelector(".composer")].filter(Boolean);

  dropTargets.forEach(target => {
    target.addEventListener("dragenter", e => {
      if (!(e.dataTransfer?.types || []).includes("Files")) return;
      e.preventDefault();
      target.classList.add("drop-image-active");
    });
    ...
    target.addEventListener("dragleave", e => {
      if (!target.contains(e.relatedTarget)) {
        target.classList.remove("drop-image-active");
      }
    });
...
function updateRadioToggle() {
  if (!radioToggle) return;
  radioToggle.classList.toggle("active", Boolean(radioOn));
  radioToggle.setAttribute("aria-pressed", String(Boolean(radioOn)));
  const label = radioToggle.querySelector("span:last-child");
  if (label) label.textContent = radioOn ? "Rádio ON" : "Rádio OFF";
  if (radioIcon) radioIcon.innerHTML = icon("radio", 14);
}
...
function applyRadioState(track, newQueue, initial = false) {
  ...
  if (!track) {
    trackTitle.textContent = "Nenhuma música"; trackChannel.textContent = "Adicione uma faixa para começar"; cover.classList.remove("has-vinyl"); cover.innerHTML = '<div class="cover-letter">S</div><div class="cover-shine"></div>';
  ...
  cover.classList.add("has-vinyl"); cover.classList.add("track-changing");
  setAmbientFromThumbnail(track.thumbnail);
  cover.innerHTML = "";
  const disc = document.createElement("div"); disc.className = `vinyl-disc${track.paused ? "" : " playing"}`;
  if (track.thumbnail) { const img = document.createElement("img"); img.src = track.thumbnail; img.alt = ""; disc.append(img); }
  cover.append(disc);
  setTimeout(() => cover.classList.remove("track-changing"), 420);
...
function setAmbientFromThumbnail(url) {
  if (!url) return;
  const img = new Image(); img.crossOrigin = "anonymous"; img.onload = () => {
    try {
      const c = document.createElement("canvas"); c.width = c.height = 24; const x = c.getContext("2d", { willReadFrequently:true }); x.drawImage(img,0,0,24,24);
      const d=x.getImageData(0,0,24,24).data; let r=0,g=0,b=0,n=0;
      for(let i=0;i<d.length;i+=4){ if(d[i+3]<120) continue; r+=d[i];g+=d[i+1];b+=d[i+2];n++; }
      if(n){ r=Math.round(r/n);g=Math.round(g/n);b=Math.round(b/n); document.documentElement.style.setProperty("--ambient", `${r},${g},${b}`); }
    } catch {}
  }; img.src=url;
}
async function openRecap(){
  recapContent.innerHTML = '<div class="recap-empty">Carregando recap…</div>'; recapModal.classList.remove("hidden");
  if (!authenticated) return;
  const ok = sendWS({type:"weekly_recap"}); if(!ok) recapContent.innerHTML='<div class="recap-empty">A sala ainda está conectando.</div>';
}
function renderRecap(d){
  const stats=d.stats||{}; const topUsers=d.topUsers||[]; const topTracks=d.topTracks||[];
  recapContent.innerHTML='';
  const grid=document.createElement("div");grid.className="recap-grid";
  [[stats.plays||0,"faixas tocadas"],[stats.uniqueTracks||0,"faixas diferentes"],[stats.messages||0,"mensagens"]].forEach(([n,l])=>{const x=document.createElement("div");x.className="recap-stat";x.innerHTML=`<b>${n}</b><span>${l}</span>`;grid.append(x)});recapContent.append(grid);
  const sec1=document.createElement("div");sec1.className="recap-section";sec1.innerHTML='<h4>QUEM MAIS COLOCOU MÚSICA</h4>';
  if(!topUsers.length) sec1.innerHTML+='<div class="recap-empty">Ainda não há dados suficientes.</div>';
  topUsers.forEach((u,i)=>{const r=document.createElement("div");r.className="recap-row";r.innerHTML=`<div class="message-avatar">${i+1}</div><div><strong>${escapeHtml(u.name)}</strong><span>${u.count} ${u.count===1?'faixa':'faixas'}</span></div>`;sec1.append(r)});recapContent.append(sec1);
  const sec2=document.createElement("div");sec2.className="recap-section";sec2.innerHTML='<h4>FAIXAS MAIS REPETIDAS</h4>';
  if(!topTracks.length) sec2.innerHTML+='<div class="recap-empty">Nenhuma faixa registrada ainda.</div>';
  topTracks.forEach(t=>{const r=document.createElement("div");r.className="recap-row";r.innerHTML=`<img src="${t.thumbnail||''}" alt=""><div><strong>${escapeHtml(t.title)}</strong><span>${t.count} ${t.count===1?'vez':'vezes'}</span></div>`;sec2.append(r)});recapContent.append(sec2);
}
...
function applySameTrackState(track, transition = false) {
  ...
  try {
    ...
    playIcon.innerHTML = icon(track.paused ? "play" : "pause", 18); const vd=cover.querySelector(".vinyl-disc"); if(vd) vd.classList.toggle("playing", !track.paused); syncLabel.textContent = track.paused ? "pausado" : "ao vivo";
  } catch {}
}
